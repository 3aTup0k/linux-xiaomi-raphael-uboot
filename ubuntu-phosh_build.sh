...
# Script arguments:
# $1 - Phosh desktop variant (phosh-core | phosh-full | phosh-phone)
# $2 - kernel version (e.g. 6.18)
# $3 - language pack scope (full | minimum)
# $4 - timezone (e.g. Europe/Moscow)
# $5 - hostname (e.g. raphael)

DESKTOP_VARIANT="${1:-phosh-full}"
KERNEL_VERSION="$2"
LANGUAGE_PACK="${3:-full}"
TIMEZONE="${4:-Europe/Moscow}"
HOSTNAME="${5:-raphael}"
...
# Install base packages and Phosh desktop environment
chroot rootdir apt install -y bash-completion sudo apt-utils ssh openssh-server nano \
  systemd-boot initramfs-tools chrony curl wget dnsmasq iptables iproute2 "$DESKTOP_VARIANT"
...
if [ "$LANGUAGE_PACK" = "full" ]; then
  ...
  # Install all GNOME language packs, CJK fonts, and input methods
  ...
  # Minimum set: US English and Russian locale data + fonts
