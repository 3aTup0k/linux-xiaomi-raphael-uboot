...
# Install base packages and XFCE desktop with LightDM
chroot rootdir apt install -y bash-completion sudo apt-utils ssh openssh-server nano \
  systemd-boot initramfs-tools chrony curl wget dnsmasq iptables iproute2 \
...
if [ "$LANGUAGE_PACK" = "full" ]; then
  ...
  chroot rootdir apt install -y language-pack-\* fonts-noto-cjk-extra fonts-arphic-uming fonts-arphic-ukai
else
...
# Configure CDC NCM (USB networking) – identical to GNOME/Phosh builds
